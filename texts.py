help_text = {
    'ru': '''
Здесь у вас есть список команд:
/help - Показать список команд
/info - Показать информацию о боте
/repo - Исходный код бота
/time — Получить текущее время на планетах
/search — Поиск миссий разлома бездны
/sortie — Получить подробную информацию о текущей вылазки
/trader — Показывает время и местоположение торговца бездны
/arbitration — Получить текущую миссию арбитража
/nightwave - Показывает ежедневные и еженедельные ночные волны
/events - Показать события
/tenshin - Показать еженедельную награду теншина
/news - Последние новости варфрейм
        ''', 
    'uk': '''
Тут у вас є список команд:
/help - Показати список команд
/info - Показати інформацію бота
/repo - Вихідний код бота
/time — Отримати поточний час на планетах
/search — Пошук місій розлому безодні
/sortie — Отримати детальну інформацію про поточні вилазки
/trader — Показати час та місцезнаходження торговця безодні
/arbitration — Отримати поточну місію арбітражу
/nightwave - Показує щоденні та щотижневі нічні хвилі
/events - Показати події
/tenshin - Показати щотижневу нагороду теншина
/news - Останні новини варфрейм
        ''', 
    'en': """
Here you have a list of commands:
/help - Show list of commands
/info - Show bot information
/repo - Bot source code 
/time - Get the current time on planets
/search - Search for void fissure missions
/sortie - Get the details on the current Sortie
/trader - Shows the Void Trader time and location
/arbitration - Get current arbitration mission
/nightwave - Shows the daily and weekly nightwave acts
/events - Show events
/tenshin - Show weekly tenshin reward
/news - The latest news from warframe
        """
}

description_text = {
    'ru': 'Услуги бота:\n1 - предоставляет вам всю доступную информацию\n\nВеселитесь, Тэнно!', 
    'uk': 'Послуги бота:\n1 - надає вам усю доступну інформацію\n\nВеселіться, Тено!', 
    'en': 'Bot Services:\n1 - provides you with all available information\n\nHave fun, Tenno!'
}

error_text = {
    'ru': 'Что-то пошло не так. Пожалуйста, попытайтесь еще раз.', 
    'uk': 'Щось пішло не так. Будь ласка, спробуйте ще раз.', 
    'en': 'Something went wrong please try again.'
}

mission_text = {
    'ru': {
        0 : 'Планета', 
        1 : 'Реликвия', 
        2 : 'Окончания', 
        3 : 'Тип миссии', 
        4 : 'Враги', 
        5 : 'Равнины Эйдолона', 
        6 : 'Валлис', 
        7 : 'Камбион',
        8 : 'Выберете между',
        9 : 'Здраствуй Тенно!',
    },
    'uk': {
        0 : 'Планета', 
        1 : 'Реліквія', 
        2 : 'Закінчення', 
        3 : 'Тип місії', 
        4 : 'Вороги', 
        5 : 'Рівнини Ейдолона', 
        6 : 'Валліс', 
        7 : 'Камбіон',
        8 : 'Виберіть між',
        9 : 'Привіт Тенно!',
    },
    'en': {
        0 : 'Planet', 
        1 : 'Relic', 
        2 : 'Time-left', 
        3 : 'Mission Type', 
        4 : 'Enemys', 
        5 : 'Plains of Eidolon', 
        6 : 'Orb Vallis', 
        7 : 'Cambion Drift',
        8 : 'Select between',
        9 : 'Hi Tenno!',
    }
}

mission_names = {
    'ru': {
        0 : 'Захват',
        1 : 'Оборона',
        2 : 'Разрушение',
        3 : 'Раскопки',
        4 : 'Улей',
        5 : 'Перехват',
        6 : 'Мобильная Оборона',
        7 : 'Спасение',
        8 : 'Шпионаж',
        9 : 'Выживание',
        10 : 'Стычка',
        11 : 'Саботаж',
        12 : 'Арчвинг',
    },
    'uk': {
        0 : 'Захоплення',
        1 : 'Оборона',
        2 : 'Руйнування',
        3 : 'Розкопки',
        4 : 'Вулик',
        5 : 'Перехоплення',
        6 : 'Мобільна Оборона',
        7 : 'Порятунок',
        8 : 'Шпигунство',
        9 : 'Виживання',
        10 : 'Сутичка',
        11 : 'Саботаж',
        12 : 'Арчвінг',
    },
    'en': {
        0 : 'Capture',
        1 : 'Defense',
        2 : 'Disruption',
        3 : 'Excavation',
        4 : 'Hive',
        5 : 'Interception',
        6 : 'Mobile Defense',
        7 : 'Rescue',
        8 : 'Spy',
        9 : 'Survival',
        10 : 'Skirmish',
        11 : 'Sabotage',
        12 : 'Volatile',
    }
}

relic_names = {
    'ru': {
        0 : 'Лит',
        1 : 'Мезо',
        2 : 'Нео',
        3 : 'Акси',
        4 : 'Реквием',
    },
    'uk': {
        0 : 'Літ',
        1 : 'Мезо',
        2 : 'Нео',
        3 : 'Аксі',
        4 : 'Реквієм',
    },
    'en': {
        0 : 'Lith',
        1 : 'Meso',
        2 : 'Neo',
        3 : 'Axi',
        4 : 'Requiem',
    }
}

en_help_names = ['/time', '/search', '/sortie', '/trader', '/arbitration', '/nightwave', '/events', '/spreward']
languages = ['ru', 'uk', 'en']